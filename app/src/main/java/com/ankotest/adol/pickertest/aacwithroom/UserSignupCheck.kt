...
class UserSignupCheck(private var data: SignUpTable, private var callBack: () -> Unit) : RecyclerView.Adapter<UserSignupCheck.ViewHolder>() {
...
//        oldStatus.size.pln()
    ...
    //View ---------------------
    private fun headOB(ui: AnkoContext<UserSignupCheck>): View {
        return ui.apply {
            linearLayout {
    ...
    private fun endOB(ui: AnkoContext<UserSignupCheck>): View {
        return ui.apply {
            linearLayout {
                lparams(matchParent, dip(55))
                gravity = Gravity.CENTER
